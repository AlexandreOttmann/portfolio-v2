<template>
  <section class="mx-auto mt-4 flex max-w-4xl flex-col p-7 sm:mt-20">
    ...
    <div :class="showSearch ? '' : 'mb-3'">
      <span class="font-newsreader italic text-white-shadow cursor-pointer select-none text-lg"
    ...
    <div v-if="showSearch" class="mb-4 flex flex-col gap-2">
      <div class="my-4">
        <UInput v-model="searchedTitle" class="w-full sm:w-96" :placeholder="$t('writing.search_article')" />
      </div>
      <div v-if="tags.length > 0" class="mb-4 flex flex-wrap gap-2">
        <div v-for="tag of tags" :key="tag"
          class="hover:text-shadow-md flex cursor-pointer select-none items-center rounded-md border border-muted bg-muted px-2 py-1 text-xs transition-colors duration-100 text-shadow-sm hover:bg-border-muted sm:text-sm"
          :class="{ 'bg-zinc-700': searchedTags.includes(tag) }" @click="toggleTag(tag)">

<template>
    <div v-if="data" class="canvas-page relative h-screen w-screen overflow-hidden"
        ...
        <Canvas ref="canvasRef" :items="data?.items || []" :base-gap="50" :zoom-options="{
            minZoom: 0.4,
            maxZoom: 2.2,
            zoomFactor: 1.08,
            enableCtrl: true,
            enableMeta: true,
            enableAlt: true
        }" class="absolute inset-0" @item-click="handleItemClick">
                ...
                <Motion :initial="{
                    ...
                    <div class="absolute inset-0 rounded-2xl bg-gradient-to-br p-1 border-2 border-default/50">
                        <div class="relative size-full overflow-hidden rounded-xl">
                            <video v-if="item && isVideo(item.image)" :src="item.image" class="size-full object-cover"
                                autoplay loop muted playsinline :draggable="false" />
                            <img v-else-if="item" :src="item.image" :alt="item.title" class="size-full object-cover"
                                :draggable="false">

                            <!-- Item details overlay -->
                            <Motion v-if="!isMobile"
                                :initial="{ opacity: 0, scale: 0.95, transformOrigin: 'bottom left' }" :animate="hoveredItemIndex === index ? {
                                    opacity: 1,
                                    scale: 1,
                                ...
                                <Motion :initial="{ y: 20, opacity: 0 }" :animate="hoveredItemIndex === index ? {
                                    y: 0,
                                    opacity: 1
                                } : {
                                    y: 20,
                                    ...
                                    <div class="flex items-start gap-2">
                                        <div class="flex-1">
                                            ...
                                            <p v-if="item?.description" class="text-sm text-white/85 line-clamp-2">
                                                {{ item.description }}
                                            </p>
                                        </div>
                                        <UIcon name="i-heroicons-arrow-top-right-on-square"
                                            class="size-4 text-white/70 flex-shrink-0 mt-0.5" />
                                    </div>
                                </Motion>
                            </Motion>
                        </div>
        ...
        <div v-if="canvasRef" class="fixed bottom-2 right-2 sm:bottom-4 sm:right-4 pointer-events-none">
            <CanvasMinimap :items="data?.items || []" :grid-items="canvasRef.gridItems || []"
                :offset="canvasRef.offset || { x: 0, y: 0 }" :zoom="canvasRef.zoom || 1"
            ...
            <div class="rounded-lg bg-default/80 px-3 py-2 text-highlighted backdrop-blur-sm">
                <p class="text-xs opacity-75">
                    <span class="sm:hidden">{{ data?.items.length }} items</span><span class="hidden sm:inline">Click
                        items to
                        open links • {{ data?.items.length }} items</span>
